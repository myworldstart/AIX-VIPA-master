...
import sys
sys.path.append("..")
from asm.net.HQ_segment_anything import sam_model_registry, SamAutomaticMaskGenerator

bp = Blueprint('HQ_sam_predict', __name__)


@torch.no_grad()
@bp.route('/HQ_sam_predict', methods=['POST'])
def sam_predict():
    try:
        print(request.json)
    ...
    sam = sam_model_registry['vit_l'](checkpoint=ModelSet[modelName]['weight'])
